...
- Real-time monitoring data are posted [here](http://106.37.208.233:20035/). Historical data going back to 2013 are available for purchase from a private nonenterprise [青悦开放环境数据中心](http://data.epmap.org/). Another version of the historical data (May 2014 onward) is provided by Xiaolei Wang free of charge [here](https://quotsoft.net/air/).
...
OMSO<sub>2<sub>e is collected by OMI, a spectrometer aboard NASA’s Aura satellite. OMSO2e measures Total Column Density of SO2 in the Planetary Boundary Layer. The data can be downloaded from [NASA Earth Data](https://disc.gsfc.nasa.gov/datasets/OMSO2G_003/summary).  
*Note*: OMSO2e contains many negative values with a mean value close to zero. Negative values are a result of OMSO2e’s retrieval algorithm, which is based on principal component analysis. The algorithm may give slightly negative values in regions where SO2 emissions are low. However, when SO2 emissions are high, we will see a stronger signal and higher SO2 value.
...
- **Pollutants**: 7 major air pollutants (i.e., PM1, PM2.5, PM10, O3, NO2, SO2, and CO), PM2.5 chemical composition (i.e., SO42-, NO3-, NH4+, Cl-, BC, and OM), and ambient polycyclic aromatic hydrocarbons (PAHs), including 7 carcinogenic PAHs (i.e., BaA, Chr, BbF, BkF, BaP, DahA, IcdP).

Released by: Wei, Jing, Zhanqing Li, Alexei Lyapustin, Jun Wang, Oleg Dubovik, Joel Schwartz, Lin Sun, Chi Li, Song Liu, and Tong Zhu. 2023. “First Close Insight into Global Daily Gapless 1 Km PM2.5 Pollution, Variability, and Health Impact.” Nature Communications 14 (1): 8349.  
...
## Monitoring Data
- **Source**: National Environmental Quality Monitoring Network–Surface Water Monitoring System, [National Surface Water Monitoring System](https://szzdjc.cnemc.cn:8070/GJZ/Business/Publish/Main.html)  
...
- Papers that use this data
    - Chen, S., et al. (2023). Bank deregulation and corporate environmental performance. World Development, 161, 106106.
    - He, G., Wang, S., & Zhang, B. (2020). Watering Down Environmental Regulation in China*. The Quarterly Journal of Economics.
    ...
    - Wang, C., Wu, J., & Zhang, B. (2018). Environmental regulation, emissions and productivity: Evidence from Chinese COD-emitting manufacturers. Journal of Environmental Economics and Management.
    ...
    - Zhang, B., Chen, X., & Guo, H. (2018). Does central supervision enhance local environmental enforcement? Quasi-experimental evidence from China. Journal of Public Economics, 164, 70–90.
...
- The data could be downloaded from [中国气象数据网](https://data.cma.cn/) before 2021, with access as an education user.
...
CMDSC only has real-time data available to the public; historical data going back to 2017 are available for purchase from a private nonenterprise http://data.epmap.org/ (青悦开放环境数据中心).
...
# VI. Flood
## Flood Events Data #1
Source: Global Active Archive of Large Flood Events or Global Flood Database (GFD) generated by the Dartmouth Flood Observatory (DFO). 
Coverage: Global, 1985-2021
Including location (shp file), start and end date, severity (quantified by damage, displaced/deaths), and main causes of large flood events across the globe. The data can be downloaded from https://floodobservatory.colorado.edu/Archives/index.html. 
